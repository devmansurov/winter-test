...
use Pp\Kistochki\Models\Info;
use Pp\Kistochki\Classes\Api\Resources\InfoResource;
...
class Infos extends Controller
{
    ...
    public function all(Request $request)
    {
        ...
        $resource = Info::where('status', true)->when(count($with), function ($query) use ($with) {
...
//        dump($resource);
        $res = InfoResource::collection($resource)
            ->only($fields)
            ->hide($hideFields);
//        dump($res);
        return $res;
