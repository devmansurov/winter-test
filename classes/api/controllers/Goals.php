...
use Pp\Kistochki\Classes\Api\Resources\GoalResource;
...
class Goals extends Controller
{
    public function all(Request $request)
    {
        ...
        $goals = Goal::when(count($city), function ($query) use ($city) {
    ...
    public function item(Request $request, $item)
    {
        ...
        $goal = Goal::when(count($with), function ($query) use ($with) {
            return $query->with($with);
        })->where('id', $item)
            ->orWhere('slug', $item)
            ->firstOrFail();
